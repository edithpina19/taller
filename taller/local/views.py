...
from usuarios.services import get_gemini_response


# ============================================
#            CHAT CON GEMINI (CORREGIDO)
# ============================================

@csrf_exempt
def gemini(request):
    """Recibe el mensaje del usuario y responde usando Gemini."""
    ...
    try:
        ...
        if not user_message:
            return JsonResponse({"response": "Escribe una pregunta para ayudarte."})

        # Llamar al modelo Gemini
        respuesta = get_gemini_response(user_message)

        if not respuesta:
            return JsonResponse(
                {"response": "Error: no se pudo obtener respuesta del servidor."},
                status=500
            )
